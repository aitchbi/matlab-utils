function spm_reslice_hb(P,flags,silentMode)
...
for i = 1:numel(P)
    ...
    if read_vol
        ...
        if write_vol
            VO         = P(i);
            %HB------------------------------------------------------------
            if isfield(flags,'writedirectory')
                [~,~] = mkdir(flags.writedirectory);
                [~,d1,d2] = fileparts(P(i).fname);
                d2 = fullfile(flags.writedirectory,[d1,d2]);
                VO.fname = spm_file(d2, 'prefix',flags.prefix);
            else
