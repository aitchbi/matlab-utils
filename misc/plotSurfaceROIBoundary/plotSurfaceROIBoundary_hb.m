...
function [p,boundary_plot,BOUNDARY] = plotSurfaceROIBoundary_hb(surface,vertex_id,data,boundary_method,cmap,varargin)
...
if isempty(opts.ColorbarLimits)
    climits = [nanmin(data) nanmax(data)];
else
    climits = opts.ColorbarLimits;
end
...
switch boundary_method
    case {'midpoint','centroid','edge_vertices','edge_faces'}
        for i = 1:length(BOUNDARY)
           boundary_plot.boundary(i) = plot3(...
               BOUNDARY{i}(:,1),...
               BOUNDARY{i}(:,2),...
               BOUNDARY{i}(:,3),...
