function [cOrds,e,G,Gc] = hb_cheby_order_est(g,arange,opts)
...
while true
    ...
    I = find(d1>=tolf);
    I = I(randperm(length(I))); % Note 1
    d2 = false(size(y));
    nn = 1:Ng;
    for iI=1:length(I)
        d3 = x(I(iI));
        for n=nn(:)' % enforce row vector
            if g{n}(d3)>1e-3
                d2(n) = true;
            end
        end
...
% Note1: should enable faster search across kernels in for loop below. 
end
